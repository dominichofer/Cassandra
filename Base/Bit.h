...
// Workaround for nvcc's missing standard library "bit"
#ifndef __NVCC__
	#include <bit>
#else
namespace std
...
CUDA_CALLABLE inline void ClearLSB(uint64_t& b) noexcept
{
	b &= b - 1;
}

// Extract bit field of length 'len' starting at bit 'start' from 'src'.
...
// Extract bits from 'src' at bit indices specified by 'mask'.
