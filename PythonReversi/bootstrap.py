...
if __name__ == '__main__':
    ...
    #        solve_game_score_file(edax, f, d, d)
    #        stop = timeit.default_timer()
    #        print(f'{d} {f} {stop - start}')
    #quit()

    engines = [
        edax_engine(0),
        cassandra_engine(r'G:\Reversi2\blocksize5_it1.model', 0),
        cassandra_engine(r'G:\Reversi2\blocksize5_it2.model', 0),
        ]
    names = [
        'Edax4.4 level 0',
        'Ours Iteration 1',
    ...
    for engine in engines:
        ...
        start = timeit.default_timer()
        games = self_play(engine, games)
        stop = timeit.default_timer()
        print(f'self play {name}: {stop - start}')

        write_game_file(games, fr'G:\Reversi2\{name}_vs_{name}_frome54.game')
