...
uint64_t StableCornersOpponent(const Position& pos)
{
	uint64_t right_corners = (pos.Opponent() & 0x0100000000000001ULL) << 1;
	uint64_t left_corners = (pos.Opponent() & 0x8000000000000080ULL) >> 1;
	uint64_t upper_corners = (pos.Opponent() & 0x8100000000000081ULL) << 8;
	uint64_t lower_corners = (pos.Opponent() & 0x8100000000000081ULL) >> 8;
	return (right_corners | left_corners | upper_corners | lower_corners | 0x8100000000000081ULL) & pos.Opponent();
}
