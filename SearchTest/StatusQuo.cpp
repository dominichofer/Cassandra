...
//   [-inf, score <= alpha]. This indicates that all moves were below the window of interest.
//   [score, score], with alpha < score < beta. This indicates that the best move was within the window of interest and was equal to score.
//   [score >= beta, +inf]. This indicates that one move was above the window of interest.
//
// SCORE <= ALPHA:
//      alpha  beta
// ------(------)------> window of interest
...
// {ExactScore, MinBound, MaxBound} x {low score, mid score, high score} x {requested selectivity : 2} x {move selectivity : 2} x {score : 129} x {depth : 3}
...
constexpr Selectivity selectivity(1.0);
Intensity intensity{ {alpha, beta}, depth, selectivity };
...
namespace Update
{
	TEST(StatusQuo_ImproveWithMove, low_low)
	{
		for (int lower : range(min_score, alpha + 1))
			for (int upper : range(lower, alpha + 1))
			{
				...
				Result novum = Result({ lower, upper }, intensity.depth - 1, intensity.selectivity, Field::A1, 10 /*nodes*/);
				...
				EXPECT_EQ(status_quo.GetResult().selectivity, intensity.selectivity);
	...
	TEST(StatusQuo_ImproveWithMove, CutTest_mid_produces_no_result_and_updates_internals)
	{
		for (int score : range(alpha + 1, beta))
		{
			...
			Result novum = Result::ExactScore(score, intensity.depth - 1, intensity.selectivity, Field::A1, 10 /*nodes*/);
			status_quo.Improve(novum, Field::A2);
	...
	TEST(StatusQuo_ImproveWithMove, CutTest_high_produces_result)
	{
		for (int lower : range(beta, max_score + 1))
			for (int upper : range(lower, max_score + 1))
			{
				...
				EXPECT_EQ(result.selectivity, intensity.selectivity);
	...
	TEST(StatusQuo_ImproveWithMove, CutTest_exact_full_window_search_produces_no_result)
	{
		for (int score : range(min_score, max_score))
		{
			...
			status_quo.Improve(Result::ExactScore(score, intensity.depth - 1, intensity.selectivity, Field::invalid, 1 /*node_count*/), Field::A1);
...
namespace SingleMove
{
	TEST(StatusQuo_ImproveWithMove, SingleMove_low)
	{
		for (int lower : range(min_score, alpha + 1))
			for (int upper : range(lower, alpha + 1))
			{
				...
				// This simulates a child returning from a beta cut.
				status_quo.Improve(Result({ lower, upper }, intensity.depth - 1, intensity.selectivity, Field::invalid, 1 /*nodes*/), Field::A1);
	...
	TEST(StatusQuo_ImproveWithMove, SingleMove_mid)
	{
		for (int score : range(alpha + 1, beta))
		{
			...
			status_quo.Improve(Result::ExactScore(score, intensity.depth - 1, intensity.selectivity, Field::invalid, 1 /*nodes*/), Field::A1);
...
TEST(StatusQuo_ImproveWithMove, TwoMoves_mid_lower_first)
{
	for (int lower : range(alpha + 1, beta))
		for (int upper : range(lower + 1, beta))
		{
			...
			status_quo.Improve(Result::ExactScore(lower, intensity.depth - 1, intensity.selectivity, Field::invalid, 1 /*nodes*/), Field::A1);
			status_quo.Improve(Result::ExactScore(upper, intensity.depth - 1, intensity.selectivity, Field::invalid, 1 /*nodes*/), Field::A2);
...
TEST(Limits_Improve, CutTest_exact_always_cuts)
{
	for (int score : range(min_score, max_score + 1))
	{
		Limits limits(intensity);
		Result novum = Result::ExactScore(score, 28, Selectivity(2.4), Field::A1, 10 /*nodes*/);
		...
		EXPECT_EQ(limits.GetResult().selectivity, novum.selectivity);
		EXPECT_EQ(limits.GetResult().best_move, novum.best_move);
		EXPECT_EQ(limits.GetResult().node_count, novum.node_count + 1);
	}
}

TEST(Limits_Improve, CutTest_low_low)
{
	for (int lower : range(min_score, alpha + 1))
		for (int upper : range(lower, alpha + 1))
		{
			Limits limits(intensity);
			Result novum({ lower, upper }, 28, Selectivity(2.4), Field::A1, 10 /*nodes*/);
...
//TEST(Limits_Improve, CutTest_low_mid_overlap)
//{
//	for (int lower : range(min_score, alpha + 1))
//		for (int upper : range(alpha + 1, beta))
//		{
//			StatusQuo status_quo(intensity);
//			Result result({ lower, upper }, 28, Selectivity(2.4), Field::A1, 1 /*nodes*/);
//
...
//			EXPECT_EQ(status_quo.GetResult().selectivity, result.selectivity);
//			EXPECT_EQ(status_quo.GetResult().node_count, 1);
//		}
//}
//
//TEST(Limits_Improve, CutTest_low_high)
//{
//	for (int lower : range(min_score, alpha + 1))
...
//			EXPECT_EQ(limits.worst_selectivity, result.selectivity);
//			EXPECT_EQ(limits.node_count, 1);
//		}
//}
//
//TEST(Limits_Improve, CutTest_mid_high)
//{
//	for (int lower : range(alpha + 1, beta))
//		for (int upper : range(beta, max_score + 1))
//		{
...
TEST(Limits_Improve, CutTest_high_high)
{
	for (int lower : range(beta, max_score + 1))
		for (int upper : range(lower, max_score + 1))
		{
