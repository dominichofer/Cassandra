...
public:
	...
	PinnedVector(const DeviceVector<T>& o, chronosity c = syn) : PinnedVector(o.size()) { assign(o, c); }
	...
	PinnedVector<T>& operator=(const DeviceVector<T>& o) { store(o, syn); return *this; }
	...
	void assign(const DeviceVector<T>&, chronosity, cudaStream_t = 0);
	...
	void store(const DeviceVector<T>&, chronosity, cudaStream_t = 0);
	void store(const std::vector<T>&);

	std::vector<T> load() const { return { begin(), end() }; }

	bool operator==(const PinnedVector<T>& o) const noexcept { return (m_size == o.size()) && std::equal(begin(), end(), o.begin(), o.end()); }
	bool operator!=(const PinnedVector<T>& o) const noexcept { return !(*this == o); }

	      T& operator[](std::size_t pos)       noexcept { return m_vec[pos]; }
	const T& operator[](std::size_t pos) const noexcept { return m_vec[pos]; }
	      T& at(std::size_t pos)       noexcept(false);
	const T& at(std::size_t pos) const noexcept(false);
	      T& front()       noexcept { return m_vec[0]; }
	const T& front() const noexcept { return m_vec[0]; }
	      T& back()       noexcept { return m_vec[m_size - 1]; }
	const T& back() const noexcept { return m_vec[m_size - 1]; }
...
template<typename T>
void PinnedVector<T>::assign(const DeviceVector<T>& src, chronosity c, cudaStream_t stream)
{
	assert(m_capacity >= src.size());
...
template<typename T>
void PinnedVector<T>::store(const DeviceVector<T>& src, chronosity c, cudaStream_t stream)
{
	if (m_capacity < src.size())
		swap(PinnedVector<T>{ src.size() });
	assign(src, c, stream);
