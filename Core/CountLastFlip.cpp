...
{
	int move = static_cast<int>(f);
	int	x = move % 8;
	int	y = move / 8;

	uint64_t P = pos.P & mask_x[move][3]; // mask out unrelated bits to make dummy 0 bits for outside

	return COUNT_FLIP[x][BExtr(P, move & 0xF8, 8)]
		+ COUNT_FLIP[y][PExt(P, mask_x[move][0])]
		+ COUNT_FLIP[y][PExt(P, mask_x[move][1])]
		+ COUNT_FLIP[y][PExt(P, mask_x[move][2])];
}
